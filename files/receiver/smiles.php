...
if (isset($_FILES) && ajax() == true) {
  ...
  if (db::get_column("SELECT COUNT(*) FROM `SMILES_DIR` WHERE `ID` = ? LIMIT 1", [$id_dir]) == 0) {
  ...
  for ($i = 0; $i < $fileCount; $i++) {
    ...
    if ($width > 800) {
      ...
      if (@copy($TempName, $uploadDir.$FileName.'.'.$Ext)) {
        ...
        $ID = db::get_add("INSERT INTO `SMILES` (`ACT`, `ID_DIR`, `EXT`) VALUES (?, ?, ?)", [0, $id_dir, $Ext]);        
